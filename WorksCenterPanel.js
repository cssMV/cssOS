...
exports.default = WorksCenterPanel;
...
async function apiPublishWork(id) {
    console.log("publish", id);
}
async function apiArchiveWork(id) {
...
function WorksCenterPanel({ tier }) {
    ...
    return (<div style={{
            minHeight: "100%",
            color: "#eaf4ff",
            background: "radial-gradient(1000px 500px at 0% -10%, rgba(67,170,255,.24), transparent 55%), radial-gradient(900px 500px at 100% 10%, rgba(69,243,176,.18), transparent 50%), linear-gradient(180deg,#03060d,#070d17 40%,#080e1a)",
        ...
        <div className="ws-card ws-glow">
          <div className="ws-card-inner">
            ...
            <div style={{ marginTop: 12, display: "grid", gap: 10 }}>
              {goals.map((g) => (<div key={g.id} className="ws-work-item">
      ...
      <div className="ws-card" style={{ marginTop: 14 }}>
        <div className="ws-card-inner">
          ...
          <div style={{ marginTop: 12, display: "grid", gap: 10 }}>
            {works.map((w) => (<div key={w.id} className="ws-work-item">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                  <div>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <ActionBtn tone="solid" disabled={busyId === w.id || w.status === "published" || w.status === "draft"} onClick={() => publish(w.id)}>
                      {busyId === w.id ? "处理中" : "发布"}
                    </ActionBtn>
...
//# sourceMappingURL=WorksCenterPanel.js.map