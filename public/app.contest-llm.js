...
(function () {
  ...
  async function decorateEntry(el) {
    ...
    try {
      ...
      badge.title = T("LLM judge score");
      badge.innerHTML = `🤖 ${T("Score")}: ${Number(score.overall).toFixed(1)}`;
      const bubble = document.createElement("div");
      bubble.className = "cssos-contest-llm-bubble";
      bubble.hidden = true;
      const parts = [];
      if (score.creativity != null) parts.push(`${T("Creativity")}: ${Number(score.creativity).toFixed(1)}`);
      if (score.craft != null)      parts.push(`${T("Craft")}: ${Number(score.craft).toFixed(1)}`);
      if (score.relevance != null)  parts.push(`${T("Relevance")}: ${Number(score.relevance).toFixed(1)}`);
  ...
  window.cssosRenderContestLeaderboard = async function (container, contestId) {
    ...
    container.innerHTML = (globalThis.cssosSkeletonRowsMarkup
      ? globalThis.cssosSkeletonRowsMarkup(5, T("Loading leaderboard…"))
      : `<div style="opacity:0.6;font-size:12px;">${T("Loading leaderboard…")}</div>`);
    try {
      const r = await fetch(`/api/contests/${encodeURIComponent(contestId)}/leaderboard`, { credentials: "include" });
      const j = await r.json();
      if (!j || !j.ok) {
        container.innerHTML = `<div style="opacity:0.6;font-size:12px;">${T("Failed to load.")}</div>`;
        return;
      }
      const rows = j.leaderboard || [];
      if (!rows.length) {
        container.innerHTML = `<div style="opacity:0.6;font-size:12px;">${T("No entries yet.")}</div>`;
        return;
      }
