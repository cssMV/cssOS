...
pub fn execute(build_dir: &Path, plan: &ProviderPlan) -> Result<ProviderExecution> {
    ...
    let payload_path = build_dir.join("audio_provider_eastwest.payload.json");
    let payload = json!({
        "provider": "eastwest",
        "engine": "opus",
        "library": plan.pack,
        ...
        "render_target": "hollywood-orchestra-stub"
    });
    let render_args = vec![
        "--payload".to_string(),
    ...
    Ok(ProviderExecution {
        ...
        render_bin: Some("eastwest-render".to_string()),
