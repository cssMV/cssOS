...
pub fn execute(build_dir: &Path, plan: &ProviderPlan) -> Result<ProviderExecution> {
    ...
    let payload_path = build_dir.join("audio_provider_spitfire.payload.json");
    let payload = json!({
        "provider": "spitfire",
        "engine": "spitfire-player",
        "library": plan.pack,
        "preset": plan.preset,
    ...
    let render_args = vec![
        ...
        mapped_articulation,
        "--style".to_string(),
        plan.style_hint.clone(),
        "--mix-preset".to_string(),
        payload["profile"]["default_mix_preset"]
            .as_str()
            .unwrap_or("air-lyndhurst")
            .to_string(),
        "--section-template".to_string(),
        payload["section_templates"][2]["section_name"]
            .as_str()
            .unwrap_or("chorus")
            .to_string(),
        "--keyswitch".to_string(),
    ...
    Ok(ProviderExecution {
        ...
        render_bin: Some("spitfire-render".to_string()),
