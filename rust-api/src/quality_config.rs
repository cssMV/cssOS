...
pub struct QualityGateConfig {
    ...
    pub min_melodic_voiced_ratio: f64,
    pub min_melodic_pitch_classes: usize,
    pub min_melodic_repeat_score: f64,
    pub max_av_duration_delta_s: f64,
    pub max_subtitles_audio_delta_s: f64,
    pub min_adapted_lyrics_line_delta_ratio: f64,
}
...
impl Default for QualityGateConfig {
    fn default() -> Self {
        Self {
            ...
            min_melodic_voiced_ratio: 0.35,
            min_melodic_pitch_classes: 3,
            min_melodic_repeat_score: 0.18,
            max_av_duration_delta_s: 0.5,
            max_subtitles_audio_delta_s: 1.0,
            min_adapted_lyrics_line_delta_ratio: 0.6,
        }
    }
}
...
pub fn load_quality_config() -> QualityGateConfig {
    ...
    QualityGateConfig {
        ...
        min_melodic_voiced_ratio: std::env::var("CSS_GATE_MIN_MELODIC_VOICED_RATIO")
            .ok()
            .and_then(|x| x.parse().ok())
            .unwrap_or(d.min_melodic_voiced_ratio),
        min_melodic_pitch_classes: std::env::var("CSS_GATE_MIN_MELODIC_PITCH_CLASSES")
            .ok()
            .and_then(|x| x.parse().ok())
            .unwrap_or(d.min_melodic_pitch_classes),
        min_melodic_repeat_score: std::env::var("CSS_GATE_MIN_MELODIC_REPEAT_SCORE")
            .ok()
            .and_then(|x| x.parse().ok())
            .unwrap_or(d.min_melodic_repeat_score),
        ...
        min_adapted_lyrics_line_delta_ratio: std::env::var(
            "CSS_GATE_MIN_ADAPTED_LYRICS_LINE_DELTA_RATIO",
        )
        .ok()
        .and_then(|x| x.parse().ok())
        .unwrap_or(d.min_adapted_lyrics_line_delta_ratio),
    }
}
