...
fn bump_event(
    ...
    let event_kind = match event_type {
        EventType::Heartbeat => crate::events::EventKind::Heartbeat,
        EventType::Timeout => crate::events::EventKind::Timeout,
        EventType::Cancelled => crate::events::EventKind::Cancelled,
        EventType::GateFail => crate::events::EventKind::Gate,
        EventType::StageEnd => match kind {
            Some(BadKind::Gate) => crate::events::EventKind::Gate,
            Some(BadKind::Failed) => crate::events::EventKind::Failed,
            Some(BadKind::Cancelled) => crate::events::EventKind::Cancelled,
...
fn infer_event(_stage: &str, rec: &StageRecord) -> (EventType, Option<BadKind>, serde_json::Value) {
    if matches!(rec.status, StageStatus::FAILED) {
        if rec.error_code.as_deref() == Some("TIMEOUT_KILLED") {
            return (
                EventType::Timeout,
                Some(BadKind::Failed),
                serde_json::json!({ "timeout_s": rec.timeout_seconds.unwrap_or_default() }),
            );
        }
        if rec.error_code.as_deref() == Some("CANCELLED_KILLED")
            || rec.error_code.as_deref() == Some("CANCELLED")
        {
            return (
                EventType::Cancelled,
                None,
        ...
        let is_gate = stage_has_gate_meta(rec)
            || rec
                .error
                .as_deref()
                .map(|e| e.starts_with("OUTPUT_INVALID:"))
                .unwrap_or(false);
        if is_gate {
            return (EventType::GateFail, Some(BadKind::Gate), rec.meta.clone());
        }
        return (
            EventType::StageEnd,
            Some(BadKind::Failed),
            serde_json::json!({ "exit_code": rec.exit_code.unwrap_or(-1) }),
        );
    }
    (
        EventType::StageEnd,
        None,
        serde_json::json!({ "exit_code": rec.exit_code.unwrap_or(0) }),
    )
}

fn apply_cancel_convergence(st: &mut RunState) -> bool {
    ...
    if changed {
        ...
        bump_event(
            ...
            "CANCELLED".to_string(),
            Some(BadKind::Failed),
            serde_json::json!({"sweep_pending": swept}),
        );
    }
    changed
}
    ...
    for attempt in 0..=max_retries {
        ...
        match run_cmd_async(name, cmdline, Some(run_out_dir), timeout_s, Some(state_path)).await {
            ...
            Ok((code, _stdout, stderr, killed, timed_out, pid, pgid)) => {
                ...
                if timed_out {
                    rec.meta = serde_json::json!({"timeout_s": timeout_s.unwrap_or_default()});
                } else if killed {
                    rec.meta = serde_json::json!({"cancelled": true});
                }
                rec.pid = None;
