...
OUTPUT_DIR = Path("data/meta/day3_triage/pending_shortlist")
OUTPUT_CSV = OUTPUT_DIR / "day3_pending_shortlist.csv"
...
def main() -> None:
    ...
    with INPUT.open("r", encoding="utf-8", newline="") as handle:
        rows = [row for row in csv.DictReader(handle) if (row.get("status") or "").strip().lower() == "pending"]
    ...
    for character_id in sorted(grouped):
        ...
        sheet_path = OUTPUT_DIR / f"{character_id}_pending_contact_sheet.jpg"
        if image_paths:
            write_contact_sheet(image_paths, sheet_path)

        list_path = OUTPUT_DIR / f"{character_id}_pending.txt"
        with list_path.open("w", encoding="utf-8") as handle:
            for item in items:
                handle.write(
                    f"{item['sample_index']}\t{item['source_name']}\t{item['video_path']}\n"
                )
