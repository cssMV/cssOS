...
OUTPUT_ROOT = Path("data/meta/day5_replacement_triage/keep_shortlist")
...
def main() -> None:
    ...
    csv_out = OUTPUT_ROOT / "char002_replacement_keep_shortlist.csv"
    with csv_out.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
        writer.writeheader()
        writer.writerows(keep)

    txt_out = OUTPUT_ROOT / "char002_replacement_keep.txt"
    txt_out.write_text(
        "".join(f"{row['sample_index']} | {row['decision']} | {row['source_name']} | {row['video_path']}\n" for row in keep),
        encoding="utf-8",
    )
    ...
    write_contact_sheet(thumb_paths, OUTPUT_ROOT / "char002_replacement_keep_contact_sheet.jpg")
