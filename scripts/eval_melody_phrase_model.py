...
from train_melody_phrase_model import PhraseDataset, MelodyPhraseModel
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pitch_loss = F.cross_entropy(pitch_logits.transpose(1, 2), batch["pitch_targets"], reduction="none")
            dur_loss = F.cross_entropy(dur_logits.transpose(1, 2), batch["dur_targets"], reduction="none")
            ...
            pitch_correct += float(((pitch_pred == batch["pitch_targets"]).float() * mask_targets).sum().item())
            pitch_total += float(mask_targets.sum().item())
            dur_correct += float(((dur_pred == batch["dur_targets"]).float() * mask_targets).sum().item())
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate a trained melody phrase model on the test split.")
    ...
    dataset = PhraseDataset(Path(args.test_jsonl).expanduser().resolve())
    if len(dataset) == 0:
        raise RuntimeError("no test phrase examples found")
    ...
    metrics["schema"] = "css.melody_phrase_model.test_metrics.v1"
