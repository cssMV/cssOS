...
enum CathedralFX {
    ...
    // W1403 — Jing:「响应音量」= emoji【密度】(count, 由调用方按能量给), 不是抖动 → 这里大小/速度恢复自然、不随能量变。
    static func musicEdgeEmoji(into root: Entity, emotion: String, count: Int,
        ...
        for i in 0..<max(1, count) {
            ...
            let node: Entity = (mesh3D ? MeshPetal3D.make(for: glyph) : nil)
                ?? emojiPlane(glyph, size: Float.random(in: 0.06...0.11))   // 自然大小, 不随能量
            ...
            let dur = Double.random(in: 2.2...3.6)   // 自然速度, 不随能量
